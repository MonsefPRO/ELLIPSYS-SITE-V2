...
import { Building2, Home, Sun, MapPin, Clock, Target, ChevronRight } from "lucide-react";
          ...
          <div className="bg-white rounded-[2rem] overflow-hidden shadow-xl border border-slate-100">
            ...
            <div className="p-6 md:p-8">
              ...
              <p className="text-sm text-slate-600 leading-relaxed mb-6">
                ...
                <strong>{isEn ? "Our approach:" : "Notre action :"}</strong> {isEn ? <>Deployment of our <strong>remote-controlled robot</strong> (soft rotary brushes, no detergent) across the arrays. Uniform cleaning, no micro-scratches, zero work at height. The before/after above speaks for itself: panels back to their rated efficiency.</> : <>Déploiement de notre <strong>robot télécommandé</strong> (brosses rotatives souples, sans détergent) sur les rangées de panneaux. Nettoyage homogène, sans micro-rayures, zéro travail en hauteur. L&apos;avant/après ci-dessus parle de lui-même : des panneaux qui retrouvent leur rendement nominal.</>}
              </p>
              <Link href="/prestations/nettoyage-solaire" className="inline-flex items-center gap-2 font-bold text-brand-orange-500 hover:gap-4 transition-all text-sm mt-4">
                {isEn ? "Discover our solar cleaning expertise" : "Découvrir l'expertise nettoyage solaire"} <ChevronRight className="w-4 h-4" />
              </Link>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2rem] overflow-hidden shadow-xl border border-slate-100">
            ...
            <div className="p-6 md:p-8">
              ...
              <p className="text-sm text-slate-600 leading-relaxed mb-6">
                ...
                <strong>{isEn ? "Our approach:" : "Notre action :"}</strong> {isEn ? <>An <strong>aerial survey</strong> first maps the site: access, row spacing, sensitive areas (drone view above). Our robot then covers up to <strong>1,200 m² per hour</strong> — the equivalent of 2 MWc cleaned in a single day.</> : <>Un <strong>repérage aérien</strong> cartographie d&apos;abord le site : accès, espacement des rangées, zones sensibles (vue drone ci-dessus). Notre robot couvre ensuite jusqu&apos;à <strong>1 200 m² par heure</strong> — l&apos;équivalent de 2 MWc nettoyés en une seule journée.</>}
              </p>
